...
impl GameEngine {
    ...
    pub fn get_full_state(&mut self) -> GameResponse {
        ...
        if !room.items.is_empty() {
            let names: Vec<String> = room.items.iter().filter_map(|id| {
                self.all_items.iter().find(|i| i.id == *id).map(|i| format!("{} {}", i.icon, i.name))
            }).collect();
            msgs.push(GameMessage { text: format!("You can see: {}", names.join(", ")), msg_type: MessageType::Description, timestamp: String::new() });
        }

        let exits: Vec<String> = room.exits.iter().filter(|e| !e.hidden).map(|e| {
            let lock = if e.locked { " [locked]" } else { "" };
            format!("{} {}{}", e.direction.arrow(), e.direction.display(), lock)
        }).collect();
        msgs.push(GameMessage { text: format!("Exits: {}", exits.join("  ")), msg_type: MessageType::Description, timestamp: String::new() });
    ...
    fn get_full_state_with_messages(&self, messages: Vec<GameMessage>) -> GameResponse {
        let room = self.rooms.iter().find(|r| r.id == self.current_room_id).unwrap();
        let now = Local::now().format("%H:%M:%S").to_string();
        let timestamped_msgs: Vec<GameMessage> = messages.into_iter().map(|mut m| {
            if m.timestamp.is_empty() { m.timestamp = now.clone(); }
            m
        }).collect();

        let ground_items: Vec<InventoryItem> = room.items.iter().filter_map(|id| {
            self.all_items.iter().find(|i| i.id == *id).map(|i| InventoryItem {
                id: i.id.clone(), name: i.name.clone(), description: i.description.clone(),
                icon: i.icon.clone(), category: format!("{:?}", i.category), usable: i.usable,
            })
        }).collect();

        let exit_states: Vec<ExitState> = room.exits.iter().filter(|e| !e.hidden).map(|e| ExitState {
        ...
        let map_rooms: Vec<MapRoom> = self.rooms.iter().map(|r| {
            let (x, y) = map_positions.get(&r.id).copied().unwrap_or((5.0, 5.0));
            MapRoom { id: r.id.clone(), name: r.name.clone(), x, y, visited: r.visited, is_current: r.id == self.current_room_id }
        }).collect();

        let map_connections: Vec<MapConnection> = self.rooms.iter().flat_map(|r| {
            r.exits.iter().filter(|e| !e.hidden).map(|e| MapConnection {
                from: r.id.clone(), to: e.target_room.clone(), direction: e.direction.display().into(),
            }).collect::<Vec<_>>()
        }).collect();

        let achievement_states: Vec<AchievementState> = self.achievements.iter().map(|a| AchievementState {
            id: a.id.clone(), name: a.name.clone(), description: a.description.clone(),
            icon: a.icon.clone(), unlocked: a.unlocked, hidden: a.hidden,
        }).collect();

        GameResponse {
            messages: timestamped_msgs,
            current_room: RoomState {
                id: room.id.clone(), name: room.name.clone(), description: room.description.clone(),
                exits: exit_states, items_on_ground: ground_items,
                lighting: room.lighting.clone(), ambient: room.ambient_sound.clone().unwrap_or_default(),
            },
            inventory: self.player.get_inventory_items(),
            player: self.player.get_state(),
            map_data: MapData { rooms: map_rooms, current_room_id: self.current_room_id.clone(), connections: map_connections },
            achievements: achievement_states,
            score: self.score, game_over: self.game_over, game_won: self.game_won,
        }
    }

    pub fn get_current_room(&self) -> &Room {
        self.rooms.iter().find(|r| r.id == self.current_room_id).unwrap()
    }
 ...
 let now = Local::now().format("%Y-%m-%d %H:%M").to_string();
        ...
        let checks: Vec<(&str, bool, &str)> = vec![
            ...
            ("cartographer", self.player.rooms_explored >= self.rooms.len() as u32, ""),
            ("collector", self.player.items_collected >= 5, ""),
            ("bookworm", self.player.inventory.iter().filter(|i| matches!(i.category, ItemCategory::Document)).count() >= 5, ""),
            ("puzzle_master", self.player.puzzles_solved >= 3, ""),
            ("codebreaker", self.global_flags.contains(&"office_code_known".to_string()), ""),
            ("chemist", self.global_flags.contains(&"stabilizer_created".to_string()), ""),
            ("engineer", self.global_flags.contains(&"generator_fixed".to_string()), ""),
            ("hacker", self.global_flags.contains(&"files_decrypted".to_string()), ""),
            ("savior", self.global_flags.contains(&"core_stabilized".to_string()), ""),
        ...
        for (id, condition, _) in checks {
            if let Some(ach) = self.achievements.iter_mut().find(|a| a.id == id) {
                if condition && !ach.unlocked {
                    ...
                    messages.push(GameMessage { text: format!("Achievement Unlocked: {} {} - {}", ach.icon, ach.name, ach.description), msg_type: MessageType::Achievement, timestamp: String::new() });
                }
            }
        }
    }
    ...
    pub fn list_save_slots(&self) -> Vec<SaveSlotInfo> {
        save::list_save_slots_info().into_iter().map(|s| SaveSlotInfo {
            slot: s.slot, exists: s.exists, room_name: s.room_name,
            play_time: s.play_time, timestamp: s.timestamp,
        }).collect()
    }
    ...
    pub fn get_achievements(&self) -> Vec<AchievementState> {
        self.achievements.iter().map(|a| AchievementState {
            id: a.id.clone(), name: a.name.clone(), description: a.description.clone(),
            icon: a.icon.clone(), unlocked: a.unlocked, hidden: a.hidden,
        }).collect()
    }
    ...
    pub fn get_autocomplete_suggestions(&self, partial: &str) -> Vec<String> {
        ...
        let base_commands = ["help", "look", "take", "drop", "use", "combine", "inventory", "examine", "read", "solve", "map", "status", "hint", "restart"];
        for cmd in &base_commands {
            if cmd.starts_with(&p) { suggestions.push(cmd.to_string()); }
        }
        let directions = ["north", "south", "east", "west", "up", "down"];
        for dir in &directions {
            if dir.starts_with(&p) { suggestions.push(dir.to_string()); }
        }
        let room = self.get_current_room();
